...
export function GrantStudentAccessForm({ studentId }: { studentId: string }) {
    ...
    return (
      <div className="rounded-md border border-amber-300 bg-amber-50 p-2 text-xs dark:border-amber-900 dark:bg-amber-950">
        <p className="font-medium">
          Temp password: <code className="rounded bg-background px-1">{state.tempPassword}</code>
        </p>
  ...
  return (
    <form action={formAction} className="flex items-center gap-2">
      ...
      <Input name="email" type="email" placeholder="student@email.com" className="h-7 w-44 text-xs" required />
